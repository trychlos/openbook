	...
	for( ili=list, i=0 ; ili ; ili=ili->next, ++i ){
		...
		if( !begin || !g_date_valid( begin )){
			...
			if( !end || !g_date_valid( end )){
				...
				send = my_utils_date_to_str( end, MY_DATE_DMMM );
			}
		} else {
			sbegin = my_utils_date_to_str( begin, MY_DATE_DMMM );
			if( !end || !g_date_valid( end )){
				send = g_strdup( "" );
			} else {
				send = my_utils_date_to_str( end, MY_DATE_DMMM );
...
display_balances( ofaJournalProperties *self )
{
	...
	str = my_utils_date_to_str( ofo_journal_get_deb_date( priv->journal, priv->exe_id, priv->dev_code ), MY_DATE_DDMM );
	gtk_label_set_text( label, str );
	g_free( str );
	...
	str = my_utils_date_to_str( ofo_journal_get_cre_date( priv->journal, priv->exe_id, priv->dev_code ), MY_DATE_DDMM );
	gtk_label_set_text( label, str );
	g_free( str );
}
