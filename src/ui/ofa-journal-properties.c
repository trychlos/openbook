...
display_balances( ofaJournalProperties *self )
{
	...
	str = my_date_to_str( ofo_journal_get_deb_date( priv->journal, priv->exe_id, priv->dev_code ), MY_DATE_DMYY );
	gtk_label_set_text( label, str );
	g_free( str );
	...
	str = my_date_to_str( ofo_journal_get_cre_date( priv->journal, priv->exe_id, priv->dev_code ), MY_DATE_DMYY );
	gtk_label_set_text( label, str );
	g_free( str );
}
