 ...
 */
struct _ofaPDFAccountsBalancePrivate {
...
static const gchar *st_ui_xml            = PKGUIDIR "/ofa-print-accounts-balance.ui";
static const gchar *st_ui_id             = "PrintAccountsBalanceDlg";

static const gchar *st_pref_uri          = "PDFAccountsBalanceURI";
static const gchar *st_pref_settings     = "PDFAccountsBalanceSettings";
...
#define st_number_width                  (gdouble) 49/9*body_font_size
#define st_currency_width                (gdouble) 22/9*body_font_size
...
static void     init_date_selection( ofaPDFAccountsBalance *self );
static void     init_others( ofaPDFAccountsBalance *self );
static void     on_per_class_toggled( GtkToggleButton *button, ofaPDFAccountsBalance *self );
static void     on_print_zero_toggled( GtkToggleButton *button, ofaPDFAccountsBalance *self );
...
static void     draw_account_balance( ofaIPrintable *instance, GtkPrintContext *context, GList *list, gdouble top, const gchar *title, gboolean balancing );
static GList   *add_account_balance( ofaPDFAccountsBalance *self, GList *list, const gchar *currency, gdouble solde, ofsAccountBalance *sbal );
static gint     cmp_currencies( const sCurrency *a, const sCurrency *b );
static void     get_settings( ofaPDFAccountsBalance *self );
static void     set_settings( ofaPDFAccountsBalance *self );

G_DEFINE_TYPE_EXTENDED( ofaPDFAccountsBalance, ofa_pdf_accounts_balance, OFA_TYPE_PDF_DIALOG, 0, \
...
static void
pdf_accounts_balance_finalize( GObject *instance )
{
	static const gchar *thisfn = "ofa_pdf_accounts_balance_finalize";
	ofaPDFAccountsBalancePrivate *priv;

	g_debug( "%s: instance=%p (%s)",
			thisfn, ( void * ) instance, G_OBJECT_TYPE_NAME( instance ));

	g_return_if_fail( instance && OFA_IS_PDF_ACCOUNTS_BALANCE( instance ));

	/* free data members here */
	priv = OFA_PDF_ACCOUNTS_BALANCE( instance )->priv;
	g_list_free_full( priv->totals, ( GDestroyNotify ) free_currency );

	/* chain up to the parent class */
	G_OBJECT_CLASS( ofa_pdf_accounts_balance_parent_class )->finalize( instance );
}

static void
pdf_accounts_balance_dispose( GObject *instance )
{
	g_return_if_fail( instance && OFA_IS_PDF_ACCOUNTS_BALANCE( instance ));
	...
	/* chain up to the parent class */
	G_OBJECT_CLASS( ofa_pdf_accounts_balance_parent_class )->dispose( instance );
}

static void
ofa_pdf_accounts_balance_init( ofaPDFAccountsBalance *self )
{
	static const gchar *thisfn = "ofa_pdf_accounts_balance_instance_init";
	ofaPDFAccountsBalancePrivate *priv;

	g_debug( "%s: self=%p (%s)",
			thisfn, ( void * ) self, G_OBJECT_TYPE_NAME( self ));

	g_return_if_fail( self && OFA_IS_PDF_ACCOUNTS_BALANCE( self ));

	self->priv = G_TYPE_INSTANCE_GET_PRIVATE( self, OFA_TYPE_PDF_ACCOUNTS_BALANCE, ofaPDFAccountsBalancePrivate );
	priv = self->priv;
...
static void
ofa_pdf_accounts_balance_class_init( ofaPDFAccountsBalanceClass *klass )
{
	static const gchar *thisfn = "ofa_pdf_accounts_balance_class_init";

	g_debug( "%s: klass=%p", thisfn, ( void * ) klass );

	G_OBJECT_CLASS( klass )->dispose = pdf_accounts_balance_dispose;
	G_OBJECT_CLASS( klass )->finalize = pdf_accounts_balance_finalize;

	MY_DIALOG_CLASS( klass )->init_dialog = v_init_dialog;
	MY_DIALOG_CLASS( klass )->quit_on_ok = v_quit_on_ok;

	g_type_class_add_private( klass, sizeof( ofaPDFAccountsBalancePrivate ));
}

static void
iprintable_iface_init( ofaIPrintableInterface *iface )
{
	static const gchar *thisfn = "ofa_pdf_accounts_balance_iprintable_iface_init";
...
/**
 * ofa_pdf_accounts_balance_run:
 * @main: the main window of the application.
 *
 * Print the accounts balance
 */
gboolean
ofa_pdf_accounts_balance_run( ofaMainWindow *main_window )
{
	static const gchar *thisfn = "ofa_pdf_accounts_balance_run";
	ofaPDFAccountsBalance *self;
	gboolean printed;
...
{
	get_settings( OFA_PDF_ACCOUNTS_BALANCE( dialog ));

	init_date_selection( OFA_PDF_ACCOUNTS_BALANCE( dialog ));
	init_others( OFA_PDF_ACCOUNTS_BALANCE( dialog ));
}

static void
init_date_selection( ofaPDFAccountsBalance *self )
{
	ofaPDFAccountsBalancePrivate *priv;
	GtkWindow *toplevel;
	GtkWidget *widget;
...
static void
init_others( ofaPDFAccountsBalance *self )
{
	ofaPDFAccountsBalancePrivate *priv;
	GtkWindow *toplevel;
	GtkWidget *widget;
	...
	widget = my_utils_container_get_child_by_name( GTK_CONTAINER( toplevel ), "print-zero" );
	g_return_if_fail( widget && GTK_IS_CHECK_BUTTON( widget ));
	g_signal_connect( G_OBJECT( widget ), "toggled", G_CALLBACK( on_print_zero_toggled ), self );
	gtk_toggle_button_set_active( GTK_TOGGLE_BUTTON( widget ), priv->also_zero );
	on_print_zero_toggled( GTK_TOGGLE_BUTTON( widget ), self );
}

static void
on_per_class_toggled( GtkToggleButton *button, ofaPDFAccountsBalance *self )
{
	ofaPDFAccountsBalancePrivate *priv;
	gboolean bvalue;
...
static void
on_print_zero_toggled( GtkToggleButton *button, ofaPDFAccountsBalance *self )
{
	ofaPDFAccountsBalancePrivate *priv;
	gboolean bvalue;
...
v_quit_on_ok( myDialog *dialog )
{
	...
	/* chain up to the parent class */
	ok = MY_DIALOG_CLASS( ofa_pdf_accounts_balance_parent_class )->quit_on_ok( dialog );

	if( ok ){
		ok &= do_apply( OFA_PDF_ACCOUNTS_BALANCE( dialog ));
	}
 ...
 */
static gboolean
do_apply( ofaPDFAccountsBalance *self )
{
	ofaPDFAccountsBalancePrivate *priv;
...
iprintable_get_dataset( const ofaIPrintable *instance )
{
	...
	dataset = ofo_entry_get_dataset_for_print_balance(
						dossier,
						NULL, NULL, ofo_dossier_get_exe_begin( dossier ), &priv->to_date );
...
{
	static const gchar *thisfn = "ofa_pdf_accounts_balance_iprintable_on_begin_print";
	ofaPDFAccountsBalancePrivate *priv;
	gdouble page_width;
	gint body_font_size;
...
iprintable_get_page_header_subtitle( const ofaIPrintable *instance )
{
	...
	if( my_date_is_valid( &priv->to_date )){
		...
		stitle = g_string_append( stitle, _( "At date of printing" ));
	}
		...
		GtkPrintOperation *operation, GtkPrintContext *context, gint page_num )
{
	...
	ofa_iprintable_set_text( instance, context,
			priv->body_debit_period_rtab, y-1, _( "Soldes" ), PANGO_ALIGN_CENTER );

	ofa_iprintable_set_text( instance, context,
			priv->body_debit_solde_rtab, y-1, _( "Balance" ), PANGO_ALIGN_CENTER );
...
iprintable_draw_line( ofaIPrintable *instance, GtkPrintOperation *operation, GtkPrintContext *context, GList *current )
{
	...
	priv->subtotals = add_account_balance(
						OFA_PDF_ACCOUNTS_BALANCE( instance ), priv->subtotals, sbal->currency, solde, sbal );

	priv->totals = add_account_balance(
						OFA_PDF_ACCOUNTS_BALANCE( instance ), priv->totals, sbal->currency, solde, sbal );
}
...
iprintable_draw_group_footer( ofaIPrintable *instance, GtkPrintOperation *operation, GtkPrintContext *context )
{
	...
	str = g_strdup_printf( _( "Class %d accounts balance : "), priv->class_num );
...
iprintable_draw_bottom_summary( ofaIPrintable *instance, GtkPrintOperation *operation, GtkPrintContext *context )
{
	...
	draw_account_balance( instance, context, priv->totals, top, _( "General balance : " ), FALSE );
...
draw_subtotals_balance( ofaIPrintable *instance, GtkPrintOperation *operation, GtkPrintContext *context, const gchar *title )
{
	...
	draw_account_balance( instance, context, priv->subtotals, last_y, title, TRUE );
...
draw_account_balance( ofaIPrintable *instance, GtkPrintContext *context,
							GList *list, gdouble top, const gchar *title, gboolean balancing )
{
	ofaPDFAccountsBalancePrivate *priv;
	GList *it;
	gboolean first;
	...
	for( it=list, first=TRUE ; it ; it=it->next ){
		...
		solde = scur->solde_c - scur->solde_d;

		if( !balancing || solde < 0 ){
			str = my_double_to_str( balancing ? -solde : scur->solde_d );
			ofa_iprintable_set_text( instance, context,
		...
		if( !balancing || solde >= 0 ){
			str = my_double_to_str( balancing ? solde : scur->solde_c );
			ofa_iprintable_set_text( instance, context,
...
static GList *
add_account_balance( ofaPDFAccountsBalance *self, GList *list,
							const gchar *currency, gdouble solde, ofsAccountBalance *sbal )
{
	static const gchar *thisfn = "ofa_pdf_accounts_balance_add_account_balance";
	GList *it;
	sCurrency *scur;
	gboolean found;
...
/*
 * settings are:
 * to_date;per_class;also_zero;
 */
static void
get_settings( ofaPDFAccountsBalance *self )
{
	ofaPDFAccountsBalancePrivate *priv;
	GList *slist, *it;
	const gchar *cstr;
	...
	cstr = it ? it->data : NULL;
	if( my_strlen( cstr )){
		priv->also_zero = my_utils_boolean_from_str( cstr );
	}
...
static void
set_settings( ofaPDFAccountsBalance *self )
{
	ofaPDFAccountsBalancePrivate *priv;
	gchar *str, *sto;
