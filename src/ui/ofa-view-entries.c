...
display_entry( ofaViewEntries *self, GtkTreeModel *tmodel, ofoEntry *entry )
{
	...
	sdope = my_date_dto_str( ofo_entry_get_dope( entry ), MY_DATE_DDMM );
	sdeff = my_date_dto_str( ofo_entry_get_deffect( entry ), MY_DATE_DDMM );
	...
	if( d && g_date_valid( d )){
		srappro = my_date_dto_str( d, MY_DATE_DDMM );
	} else {
		srappro = g_strdup( "" );
	}
...
ofa_view_entries_display_entries( ofaViewEntries *self, GType type, const gchar *id, const GDate *begin, const GDate *end )
{
	...
	if( !self->private->dispose_has_run ){
		...
		if( begin && g_date_valid( begin )){
			str = my_date_dto_str( begin, MY_DATE_DDMM );
		...
		if( end && g_date_valid( end )){
			str = my_date_dto_str( end, MY_DATE_DDMM );
			...
			if( g_date_valid( &deff ) && g_date_compare( &deff, &dope ) >= 0 ){
				...
				if( journal ){
					...
					if( !last_close || g_date_compare( &deff, last_close ) > 0 ){
						...
						msg2 = my_date_dto_str( last_close, MY_DATE_DDMM );
						msg3 = my_date_dto_str( &deff, MY_DATE_DDMM );
