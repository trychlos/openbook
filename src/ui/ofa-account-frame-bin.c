...
static void       setup_grid( ofaAccountFrameBin *bin );
static GtkWidget *book_get_page_by_class( ofaAccountFrameBin *bin, gint class_num, gboolean create );
static GtkWidget *book_create_page( ofaAccountFrameBin *bin, gint class );
static void       book_expand_all( ofaAccountFrameBin *bin );
static void       on_book_page_switched( GtkNotebook *book, GtkWidget *wpage, guint npage, ofaAccountFrameBin *bin );
static gboolean   on_book_key_pressed( GtkWidget *widget, GdkEventKey *event, ofaAccountFrameBin *bin );
static GtkWidget *page_add_treeview( ofaAccountFrameBin *bin, GtkWidget *page );
static void       page_add_columns( ofaAccountFrameBin *bin, GtkTreeView *tview );
static gboolean   is_visible_row( GtkTreeModel *tmodel, GtkTreeIter *iter, GtkWidget *page );
static GtkWidget *get_current_treeview( const ofaAccountFrameBin *bin );
static void       on_tview_row_selected( GtkTreeSelection *selection, ofaAccountFrameBin *bin );
static void       on_tview_row_activated( GtkTreeView *tview, GtkTreePath *path, GtkTreeViewColumn *column, ofaAccountFrameBin *bin );
static gboolean   on_tview_key_pressed( GtkWidget *widget, GdkEventKey *event, ofaAccountFrameBin *bin );
static void       tview_collapse_node( ofaAccountFrameBin *bin, GtkWidget *widget );
static void       tview_expand_node( ofaAccountFrameBin *bin, GtkWidget *widget );
static void       on_tview_insert( ofaAccountFrameBin *bin );
static void       do_insert_account( ofaAccountFrameBin *bin );
static void       do_update_account( ofaAccountFrameBin *bin );
static void       on_tview_delete( ofaAccountFrameBin *bin );
static void       do_delete_account( ofaAccountFrameBin *bin );
static gboolean   delete_confirmed( ofaAccountFrameBin *bin, ofoAccount *account );
static void       on_tview_cell_data_func( GtkTreeViewColumn *tcolumn, GtkCellRenderer *cell, GtkTreeModel *tmodel, GtkTreeIter *iter, ofaAccountFrameBin *bin );
static void       tview_cell_renderer_text( GtkCellRendererText *cell, gboolean is_root, gint level, gboolean is_error );
static void       on_store_row_inserted( GtkTreeModel *tmodel, GtkTreePath *path, GtkTreeIter *iter, ofaAccountFrameBin *bin );
static void       connect_to_hub_signaling_system( ofaAccountFrameBin *bin );
static void       on_hub_new_object( ofaHub *hub, ofoBase *object, ofaAccountFrameBin *bin );
static void       on_hub_updated_object( ofaHub *hub, ofoBase *object, const gchar *prev_id, ofaAccountFrameBin *bin );
static void       on_updated_class_label( ofaAccountFrameBin *bin, ofoClass *class );
static void       on_hub_deleted_object( ofaHub *hub, ofoBase *object, ofaAccountFrameBin *bin );
static void       on_deleted_class_label( ofaAccountFrameBin *bin, ofoClass *class );
static void       on_hub_reload_dataset( ofaHub *hub, GType type, ofaAccountFrameBin *bin );
static void       select_row_by_number( ofaAccountFrameBin *bin, const gchar *number );
static void       select_row_by_iter( ofaAccountFrameBin *bin, GtkTreeView *tview, GtkTreeModel *tfilter, GtkTreeIter *iter );
 ...
 */
static void
setup_grid( ofaAccountFrameBin *bin )
{
	ofaAccountFrameBinPrivate *priv;
	gulong handler;
	...
	gtk_container_add( GTK_CONTAINER( bin ), priv->grid );
	...
	g_signal_connect( priv->notebook, "switch-page", G_CALLBACK( on_book_page_switched ), bin );
	g_signal_connect( priv->notebook, "key-press-event", G_CALLBACK( on_book_key_pressed ), bin );
	...
	handler = g_signal_connect( priv->store, "ofa-row-inserted", G_CALLBACK( on_store_row_inserted ), bin );
...
book_get_page_by_class( ofaAccountFrameBin *bin, gint class_num, gboolean bcreate )
{
	...
	if( !found && bcreate ){
		found = book_create_page( bin, class_num );
		if( !found ){
 ...
 */
static GtkWidget *
book_create_page( ofaAccountFrameBin *bin, gint class_num )
{
	...
	g_debug( "%s: bin=%p, class_num=%d", thisfn, ( void * ) bin, class_num );
...
static void
book_expand_all( ofaAccountFrameBin *bin )
{
	ofaAccountFrameBinPrivate *priv;
	gint pages_count, i;
	GtkWidget *page_w;
 ...
 */
static void
on_book_page_switched( GtkNotebook *book, GtkWidget *wpage, guint npage, ofaAccountFrameBin *bin )
{
	GtkWidget *tview;
 ...
 */
static gboolean
on_book_key_pressed( GtkWidget *widget, GdkEventKey *event, ofaAccountFrameBin *bin )
{
	ofaAccountFrameBinPrivate *priv;
	gboolean stop;
	...
	if( event->state == GDK_MOD1_MASK ||
		...
		if( class_num > 0 ){
			page_widget = book_get_page_by_class( bin, class_num, FALSE );
			if( page_widget ){
...
page_add_treeview( ofaAccountFrameBin *bin, GtkWidget *page )
{
	...
	g_signal_connect( tview, "row-activated", G_CALLBACK( on_tview_row_activated), bin );
	g_signal_connect( tview, "key-press-event", G_CALLBACK( on_tview_key_pressed ), bin );

	select = gtk_tree_view_get_selection( GTK_TREE_VIEW( tview ));
	gtk_tree_selection_set_mode( select, GTK_SELECTION_BROWSE );
	g_signal_connect( select, "changed", G_CALLBACK( on_tview_row_selected ), bin );
 ...
 */
static void
page_add_columns( ofaAccountFrameBin *bin, GtkTreeView *tview )
{
	ofaAccountFrameBinPrivate *priv;
	...
	fn_data = priv->cell_fn ? priv->cell_data : bin;
...
static GtkWidget *
get_current_treeview( const ofaAccountFrameBin *bin )
{
	ofaAccountFrameBinPrivate *priv;
	gint page_n;
	GtkWidget *page_w;
 ...
 * @book: this #ofaAccountFrameBin instance
...
static void
on_tview_row_selected( GtkTreeSelection *selection, ofaAccountFrameBin *bin )
{
	GtkTreeModel *tmodel;
	GtkTreeIter iter;
	gchar *account;
	...
	if( selection ){
		if( gtk_tree_selection_get_selected( selection, &tmodel, &iter )){
			gtk_tree_model_get( tmodel, &iter, ACCOUNT_COL_NUMBER, &account, -1 );
			g_signal_emit_by_name( bin, "ofa-changed", account );
			g_free( account );
		}
	}
...
static void
on_tview_row_activated( GtkTreeView *tview, GtkTreePath *path, GtkTreeViewColumn *column, ofaAccountFrameBin *bin )
{
	GtkTreeSelection *select;
	GtkTreeModel *tmodel;
	GtkTreeIter iter;
	...
	if( gtk_tree_selection_get_selected( select, &tmodel, &iter )){
		gtk_tree_model_get( tmodel, &iter, ACCOUNT_COL_NUMBER, &account, -1 );
		g_signal_emit_by_name( bin, "ofa-activated", account );
		g_free( account );
	}
}
 ...
 */
static gboolean
on_tview_key_pressed( GtkWidget *widget, GdkEventKey *event, ofaAccountFrameBin *bin )
{
	gboolean stop;
	...
	if( event->state == 0 ){
		if( event->keyval == GDK_KEY_Left ){
			tview_collapse_node( bin, widget );

		} else if( event->keyval == GDK_KEY_Right ){
			tview_expand_node( bin, widget );
...
static void
tview_collapse_node( ofaAccountFrameBin *bin, GtkWidget *widget )
{
	GtkTreeSelection *select;
	GtkTreeModel *tmodel;
...
static void
tview_expand_node( ofaAccountFrameBin *bin, GtkWidget *widget )
{
	GtkTreeSelection *select;
	GtkTreeModel *tmodel;
	GtkTreeIter iter;
...
static void
on_tview_insert( ofaAccountFrameBin *bin )
{
	do_insert_account( bin );
}

static void
do_insert_account( ofaAccountFrameBin *bin )
{
	ofaAccountFrameBinPrivate *priv;
	ofoAccount *account;
...
/**
 * ofa_account_frame_bin_do_new:
 */
void
ofa_account_frame_bin_do_new( ofaAccountFrameBin *bin )
{
	ofaAccountFrameBinPrivate *priv;
...
/**
 * ofa_account_frame_bin_do_properties:
 */
void
ofa_account_frame_bin_do_properties( ofaAccountFrameBin *bin )
{
	ofaAccountFrameBinPrivate *priv;
...
static void
do_update_account( ofaAccountFrameBin *bin )
{
	ofaAccountFrameBinPrivate *priv;
	ofoAccount *account;
	gchar *number;
	...
	number = ofa_account_frame_bin_get_selected( bin );
	if( number ){
...
static void
on_tview_delete( ofaAccountFrameBin *bin )
{
	ofaAccountFrameBinPrivate *priv;
	...
	account_number = ofa_account_frame_bin_get_selected( bin );
...
static void
do_delete_account( ofaAccountFrameBin *bin )
{
	ofaAccountFrameBinPrivate *priv;
	ofoAccount *account;
	gchar *number;
	...
	if( number ){
		...
		if( delete_confirmed( bin, account ) &&
			...
			ofa_account_frame_bin_set_selected( bin, number );
		}
	}
	g_free( number );
...
/**
 * ofa_account_frame_bin_do_delete:
 */
void
ofa_account_frame_bin_do_delete( ofaAccountFrameBin *bin )
{
	ofaAccountFrameBinPrivate *priv;
 ...
 */
static gboolean
delete_confirmed( ofaAccountFrameBin *bin, ofoAccount *account )
{
	gchar *msg;
	gboolean delete_ok;
 ...
 */
static void
on_store_row_inserted( GtkTreeModel *tmodel, GtkTreePath *path, GtkTreeIter *iter, ofaAccountFrameBin *bin )
{
	ofaAccountFrameBinPrivate *priv;
	gchar *number;
	gint class_num;
	...
	if( class_num != priv->prev_class ){
		book_get_page_by_class( bin, class_num, TRUE );
		priv->prev_class = class_num;
	}
}

static void
connect_to_hub_signaling_system( ofaAccountFrameBin *bin )
{
	ofaAccountFrameBinPrivate *priv;
	gulong handler;
...
on_hub_new_object( ofaHub *hub, ofoBase *object, ofaAccountFrameBin *bin )
{
	...
	if( OFO_IS_CLASS( object )){
		on_updated_class_label( bin, OFO_CLASS( object ));
	}
}
 ...
 */
static void
on_updated_class_label( ofaAccountFrameBin *bin, ofoClass *class )
{
	ofaAccountFrameBinPrivate *priv;
	GtkWidget *page_w;
	gint class_num;
	...
	page_w = book_get_page_by_class( bin, class_num, FALSE );
	if( page_w ){
...
on_hub_deleted_object( ofaHub *hub, ofoBase *object, ofaAccountFrameBin *bin )
{
	...
	if( OFO_IS_CLASS( object )){
		on_deleted_class_label( bin, OFO_CLASS( object ));
	}
}

static void
on_deleted_class_label( ofaAccountFrameBin *bin, ofoClass *class )
{
	ofaAccountFrameBinPrivate *priv;
	GtkWidget *page_w;
	gint class_num;
 ...
 */
static void
select_row_by_number( ofaAccountFrameBin *bin, const gchar *number )
{
	ofaAccountFrameBinPrivate *priv;
	GtkWidget *page_w;
	gint page_n;
	...
	if( my_strlen( number )){
		page_w = book_get_page_by_class( bin,
...
static void
select_row_by_iter( ofaAccountFrameBin *bin, GtkTreeView *tview, GtkTreeModel *tfilter, GtkTreeIter *iter )
{
	GtkTreePath *path;
...
/**
 * ofa_account_frame_bin_get_buttons_box:
 * @bin: this #ofaAccountFrameBin instance.
 *
 * Returns: the #ofaButtonsBox.
 */
ofaButtonsBox *
ofa_account_frame_bin_get_buttons_box( const ofaAccountFrameBin *bin )
{
	ofaAccountFrameBinPrivate *priv;
