...
static void
insert_row( ofaDossierStore *store, const gchar *dname )
{
	static const gchar *thisfn = "ofa_dossier_store_insert_row";
	GtkTreeIter iter;

	g_debug( "%s: store=%p, dname=%s", thisfn, ( void * ) store, dname );

	gtk_list_store_insert( GTK_LIST_STORE( store ), &iter, -1 );
	set_row( store, dname, &iter );
}

static void
set_row( ofaDossierStore *store, const gchar *dname, GtkTreeIter *iter )
{
	gtk_list_store_set(
