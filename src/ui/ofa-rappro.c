...
do_fetch( ofaRappro *self )
{
	...
	for( it=entries ; it ; it=it->next ){
		...
		sdrap = my_date_dto_str( drappro, MY_DATE_DDMM );
...
update_candidate_entry( ofaRappro *self, ofoBatLine *batline, GtkTreeIter *entry_iter )
{
	...
	sdvaleur = my_date_dto_str( dvaleur, MY_DATE_DDMM );
							...
							GtkTreeIter *entry_iter, const gchar *sdeb, const gchar *scre )
{
	...
	sdope = my_date_dto_str( dope, MY_DATE_DDMM );
...
reconciliate_entry( ofaRappro *self, ofoEntry *entry, const GDate *drappro, GtkTreeIter *iter )
{
	...
	if( is_valid_rappro ){
		str = my_date_dto_str( drappro, MY_DATE_DDMM );
	} else if( batline ){
		str = my_date_dto_str( ofo_bat_line_get_valeur( batline ), MY_DATE_DDMM );
	} else {
		str = g_strdup( "" );
	}
