...
ofa_ledger_summary_render_init( ofaLedgerSummaryRender *self )
{
	...
	g_return_if_fail( self && OFA_IS_LEDGER_SUMMARY_RENDER( self ));

	priv = ofa_ledger_summary_render_get_instance_private( self );
...
irenderable_draw_line( ofaIRenderable *instance, GList *current )
{
	...
	for( it=entries ; it ; it=it->next ){
		...
		ofs_currency_add_by_code(
				&ledger_currencies, priv->getter, ccuriso,
 ...
 */
static void
read_settings( ofaLedgerSummaryRender *self )
{
	ofaLedgerSummaryRenderPrivate *priv;
	myISettings *settings;
	GList *strlist, *it;
...
static void
write_settings( ofaLedgerSummaryRender *self )
{
	ofaLedgerSummaryRenderPrivate *priv;
	myISettings *settings;
	GtkWidget *paned;
