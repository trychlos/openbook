 ...
 * @multiple: whether the page is unique, or may be opened several times.
 *
 * Returns: the new theme identifier, strictly greater than zero.
 */
void
ofa_ipage_manager_define( ofaIPageManager *instance, GType type, const gchar *label, gboolean multiple )
{
	...
	if( OFA_IPAGE_MANAGER_GET_INTERFACE( instance )->define ){
		OFA_IPAGE_MANAGER_GET_INTERFACE( instance )->define( instance, type, label, multiple );
		return;
	}
