...
#include "core/ofa-reconcil-treeview.h"
...
tview_is_visible_entry( ofaReconcilPage *self, GtkTreeModel *tmodel, GtkTreeIter *iter, ofoEntry *entry )
{
	...
	if( DEBUG_FILTER ){
		gchar *sdeb = ofa_amount_to_str( ofo_entry_get_debit( entry ), priv->acc_currency, priv->getter );
		gchar *scre = ofa_amount_to_str( ofo_entry_get_credit( entry ), priv->acc_currency, priv->getter );
...
tview_is_visible_batline( ofaReconcilPage *self, ofoBatLine *batline )
{
	...
	if( DEBUG_FILTER ){
		gchar *samount = ofa_amount_to_str( ofo_bat_line_get_amount( batline ), priv->acc_currency, priv->getter );
...
tview_on_selection_changed( ofaTVBin *treeview, GtkTreeSelection *selection, ofaReconcilPage *self )
{
	...
	if( priv->acc_currency ){
		...
		g_free( sdeb );
		scre = ofa_amount_to_str( scur.credit, priv->acc_currency, priv->getter );
...
account_set_header_balance( ofaReconcilPage *self )
{
	...
	if( priv->account ){
		...
		priv->acc_debit = ofo_account_get_current_val_debit( priv->account )
				+ ofo_account_get_current_rough_debit( priv->account )
				+ ofo_account_get_futur_rough_debit( priv->account );
		priv->acc_credit = ofo_account_get_current_val_credit( priv->account )
...
set_reconciliated_balance( ofaReconcilPage *self )
{
	...
	if( priv->account ){
		...
		account_debit = ofo_account_get_current_val_debit( priv->account )
				+ ofo_account_get_current_rough_debit( priv->account )
				+ ofo_account_get_futur_rough_debit( priv->account );
		account_credit = ofo_account_get_current_val_credit( priv->account )
				+ ofo_account_get_current_rough_credit( priv->account )
				+ ofo_account_get_futur_rough_credit( priv->account );
...
action_on_vbat_activated( GSimpleAction *action, GVariant *empty, ofaReconcilPage *self )
{
	...
	toplevel = my_utils_widget_get_toplevel( GTK_WIDGET( self ));
	ofa_batline_properties_run( priv->getter, toplevel, priv->sel_batline );
}
