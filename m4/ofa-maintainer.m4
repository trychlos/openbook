...
#          OFA_MAINTAINER_CHECK_FOR_DEPRECATED

dnl define OFA_MAINTAINER_CHECK_MODE
dnl
...
AC_DEFUN([OFA_MAINTAINER_CHECK_MODE],[
	...
	if test "${USE_MAINTAINER_MODE}" = "yes"; then
		AC_DEFINE([OFA_MAINTAINER_MODE],[1],[Define to 1 if we are in maintainer mode])
	...
	AM_CONDITIONAL([OFA_MAINTAINER_MODE], [test "${USE_MAINTAINER_MODE}" = "yes"])
])

AC_DEFUN([OFA_MAINTAINER_CHECK_FOR_DEPRECATED],[
	AC_ARG_ENABLE(
		[deprecated],
		AC_HELP_STRING(
			[--enable-deprecated],
	...
	if test "${enable_deprecated}" = "yes"; then
		AC_DEFINE([OFA_ENABLE_DEPRECATED],[1],[Define to 1 if deprecated symbols should be enabled])
	else
		if test "${ofa_request_for_deprecated}" = "yes"; then
