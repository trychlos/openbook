...
{
	clear_tmpfiles
	# for this particular script, always display count of errors
...
function msg_help
{
	msg_version
	echo "
 This script checks for '-def.h' header files usage.
 It ensures that these definition headers are always used when they exist,
 that they are only used in other headers, and that corresponding main
 header is never used in headers.
...
function msg_version
{
 ...
 Copyright (C) 2017-2018 Pierre Wieser."
}
...
for def in $(git ls-files src | grep -e '-def\.h$' | sed -e 's|[^/]*/||'); do
	# def is 'api/ofa-igetter-def.h'
	#  should only be used in other headers
	msg "checking for ${def}..." " "
	let suberrs=0
	ack -l "^\s*#include.*${def}" src | while read inc; do
		if test "$(isHeader "${inc}")" != "h"; then
			[ ${suberrs} -eq 0 ] && echo
			let suberrs+=1
			msgerr "$def included by $inc"
		fi
	done
	main="$(echo "${def}" | sed -e 's|-def\.h$|.h|')"
	# main is 'api/ofa-igetter.h'
	#  should only be used in source files
	ack -l "^\s*#include.*${main}" src | while read inc; do
		if test "$(isHeader "${inc}")" = "h"; then
			[ ${suberrs} -eq 0 ] && echo
			let suberrs+=1
			msgerr "$main included by $inc"
		fi
	done
	[ ${suberrs} -eq 0 ] && echo "OK" || { msg "  counted ${suberrs} error(s)"; let errs+=1; }
done
